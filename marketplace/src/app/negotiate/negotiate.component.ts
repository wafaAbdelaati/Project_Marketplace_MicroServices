...
export class NegotiateComponent implements OnInit {
  ...
  validate(){
    ...
    let order = new Order(customer.id,now,"init");
    ...
    order.details.forEach(element => { 
      if(element.quantity > element.product.quantity){this.available=false;this.product+=element.product+",";}
        ...
        this.productService.updateProductQuantity(element.product.id,element.quantity).subscribe(product=>{console.log(product)});
        element.product = element.product.id;element.seller = element.seller.id;element.state="init";
        }
